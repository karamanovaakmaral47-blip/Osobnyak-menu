(function() {
    ...
    // =============================================
    // 1. ФОН НА ВСЮ СТРАНИЦУ — БЕЗ МИГАНИЯ
    ...
    if (bgContainer) {
        ...
        let bands = [];
        let builtMode = null;      // 'desktop' | 'mobile' | null — какой режим сейчас построен
        let builtCount = 0;        // сколько полос сейчас реально создано в DOM
        let photoAspect = null;
        ...
        // Строит DOM-блоки заново — вызывается ТОЛЬКО когда реально нужно
        // (первый запуск, смена ПК<->телефон, или изменилось нужное кол-во полос)
        function buildBands(fileList) {
            const frag = document.createDocumentFragment();
            bands = fileList.map(function(file) {
        ...
        function layoutDesktop() {
            ...
            if (builtMode !== 'desktop' || builtCount !== PHOTOS.length) {
                buildBands(PHOTOS);
                builtMode = 'desktop';
            }
        ...
        function layoutMobile() {
            ...
            if (builtMode !== 'mobile' || builtCount !== bandsNeeded) {
                const fileList = [];
                for (let i = 0; i < bandsNeeded; i++) {
                    fileList.push(PHOTOS[i % PHOTOS.length]);
                }
                buildBands(fileList);
                builtMode = 'mobile';
            }
